...
- `INSERT OR IGNORE` により再実行時に主キー重複でエラーにならない（**冪等**）。後段「ロールバック方針」の予防策と整合。
- `WHERE event_id IS NULL` により UPDATE も **冪等**。再実行しても新規レコードしか影響を受けない。
...
- **注意**: Drizzle スキーマで `.notNull()` を付けると `drizzle-kit generate` が物理的な NOT NULL マイグレーション（SQLite ではテーブル再作成 SQL）を自動生成してしまう。Step 3 の方針（DB は NULL 許容のまま運用）と矛盾するため避ける。
- **対処方針**:
  - Drizzle スキーマ定義では `event_id: text("event_id")` のように `.notNull()` を **付けない**（nullable のまま）。
  - 必須化は Hono の API 入力バリデーションで Zod により強制する：`eventId: z.string().min(1)`。
  - TypeScript 型は Insert / Select を分け、Insert 側のみ Zod の型推論で必須化（Drizzle の `$inferInsert` をそのまま使うと nullable のままなので、API 入力型は Zod スキーマから派生させる）。
  - これにより `CREATE → COPY → DROP → RENAME` のテーブル再作成リスクを物理 DB レイヤで負わずに済む。
