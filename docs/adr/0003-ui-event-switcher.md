...
- 値: 選択中 event の UUID
- 起動時に events 一覧をAPIで取得し、localStorage の値を読み出して URL の `:eventId` が無い場合のフォールバックに使う
- **失効時のフォールスルー**: localStorage の ID が events 一覧に含まれていない（イベント削除済み等）場合はその値を**無視**し、後続のフォールバックロジック（次項「4. URL ルーティング」）へフォールスルーする。失効した値は localStorage から削除する
...
#### 4-1. ルート `/` アクセス時のリダイレクトルール
1. localStorage に有効な（events 一覧に存在する）`event_id` があれば、その event + 既定タブへリダイレクト
2. 無効 or 未設定なら、events 一覧の先頭 event + 既定タブへリダイレクト
3. events が 0 件なら、**空状態画面**（「イベントを作成してください」CTA 付き）を表示

#### 4-2. `/events/:eventId/:tab` アクセス時のリダイレクトルール
- `:eventId` が events 一覧に存在しない → ルート `/` へリダイレクト（4-1 のロジックで再解決）
- `:tab` が `event.type` と整合しない（例: `type=meetup` なのに `:tab=tasks`）→ その event の既定タブへリダイレクト
