...
**ダイジェスト方式**: 1 実行で見つかった stale PR は「1 通のまとめメッセージ」に集約して投稿する（`📋 レビュー待ちの PR N 件 ...` ＋ 各 PR 1 行 + 依頼中レビュアーの `@メンション`）。以前は stale PR 1 件ごとに個別メッセージを投稿しており、open PR が多いとチャンネルがレビュー依頼で溢れていたため、PR が何件あっても投稿は 1 通に収まるようにした（通知は各行のメンションで従来どおり届く）。
