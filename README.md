...
`.github/workflows/` に 2 本の workflow を置いている。

- `test.yml` (CI): push / PR で発火。typecheck / vitest (miniflare D1) / FE smoke / build
  (vite build + wrangler dry-run) / E2E (Playwright) を並列ジョブで実行する。**secret 不要**
  (本番 D1 / Cloudflare / Slack には一切接続しない)。
- `deploy.yml` (CD): `main` への push (= PR マージ) で発火。D1 migration 適用 (remote) ->
  フロントビルド -> `wrangler deploy` を実行し本番へ反映する。

### CD に必要な secret（初回のみ手動登録）

CD は Cloudflare の API トークンで本番にアクセスするため、repo に以下の secret を登録する必要がある
(Settings > Secrets and variables > Actions)。**未登録の間 `deploy.yml` は赤くなる**が、
`test.yml` は影響を受けない。

| secret | 必須 | 用途 |
|---|---|---|
| `CLOUDFLARE_API_TOKEN` | 必須 | Workers / D1 の編集権限を持つ API トークン。Cloudflare ダッシュボードの My Profile > API Tokens で「Edit Cloudflare Workers」テンプレートから発行する。 |
| `CLOUDFLARE_ACCOUNT_ID` | 任意 | トークンが複数アカウントに跨る場合のみ。単一アカウントなら不要。 |

手動デプロイは従来通り `npm run deploy`（`build:frontend` + `wrangler deploy`）で可能。
