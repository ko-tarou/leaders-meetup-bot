...
/**
 * GET /api/event-actions/:actionId/roster/members
 *   includeInactive=1 で status='inactive' も含める。常に soft-deleted は除外。
 *   並び順は createdAt 昇順。
 */
rosterRouter.get("/event-actions/:actionId/roster/members", async (c) => {
...
/**
 * PUT /api/event-actions/:actionId/roster/members/:id
...
/**
 * DELETE /api/event-actions/:actionId/roster/members/:id
...
/**
 * GET /api/event-actions/:actionId/roster/columns
 *   sortOrder 昇順、同値時は createdAt 昇順。
 */
rosterRouter.get("/event-actions/:actionId/roster/columns", async (c) => {
  const db = drizzle(c.env.DB);
  const actionId = c.req.param("actionId");
  const found = await findAction(db, actionId);
  if ("error" in found) return c.json({ error: found.error }, found.status);
...
/**
 * POST /api/event-actions/:actionId/roster/columns
 ...
 */
rosterRouter.post("/event-actions/:actionId/roster/columns", async (c) => {
  const db = drizzle(c.env.DB);
  const actionId = c.req.param("actionId");
  const found = await findAction(db, actionId);
  if ("error" in found) return c.json({ error: found.error }, found.status);
...
/**
 * PUT /api/event-actions/:actionId/roster/columns/:id
...
/**
 * DELETE /api/event-actions/:actionId/roster/columns/:id
...
/**
 * DELETE /api/event-actions/:actionId/roster/members/:id/values/:columnId
