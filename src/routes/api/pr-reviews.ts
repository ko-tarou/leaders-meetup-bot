...
import { notifyReviewersAssigned } from "../../services/sticky-pr-review-board";
...
//
// Slack完結 PR3: ロジック本体は services/pr-review-actions.ts の
// reRequestReview に抽出済み（Slack sticky board ボタンと共通化）。
// この API ハンドラは薄いラッパーで、レスポンス
// (`{ ok: true, newRound }` / 404 `{ error: "Not found" }`) は不変。
prReviewsRouter.post(
  ...
  async (c) => {
    ...
    const result = await reRequestReview(c.env, { eventId, reviewId: id });
    if (!result.ok) return c.json({ error: "Not found" }, 404);

    return c.json({ ok: true, newRound: result.newRound });
  },
);
