...
// Phase 2-D: candidate_rule の正規化・候補日生成・poll 開始/締切判定の
// 純粋ロジックは src/domain/schedule/candidate-dates.ts に抽出済み。
// service は I/O (DB read / poll 作成・締切 / 集計 / 冪等) だけ担う薄い
// application フローにし、純関数は domain から re-export して既存 import
// パス (`from "../services/auto-cycle"`)・characterization テストを
// 無改変のまま維持する。値・挙動は byte-identical。
import {
  type Frequency,
  type MonthlyRule,
  type WeeklyRule,
...
} from "../domain/schedule/candidate-dates";
